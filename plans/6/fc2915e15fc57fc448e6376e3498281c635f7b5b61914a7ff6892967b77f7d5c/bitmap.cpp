  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            counter += b6.subtraction_count<bitmap_words>(v4_adj, v4_bit_idx, v4_bit_idx);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            for (size_t v4_idx = 0; v4_idx < s6.size(); v4_idx++) { // loop-4 begin
                ...
                counter += s6.subtract_cnt(v4_adj, v4_adj.vid());
            }
        }
    }
};
            ...
            if (bitmap_rows) {                                                       // bitmap-enabled continuation
                ...
                for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) { // loop-1 begin
                    ...
                    for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                        const IdType v2 = s1[v2_idx];
                        VertexSet v2_adj = query.graph->N(v2);
                        // shared bounded neighborhood projection
                        const auto &universe = bitgraph.universe();
                        const auto local_bound = universe.lower_bound(v1);
                        const auto local_row = bitgraph.row(v1);
                        Bitmap b5(universe);
                        Bitmap b4(universe);
                        b5.assign_neighbors(v2_adj.data(), v2_adj.size(), v1);
                        b4.assign_subtraction(b5, local_row, local_bound, local_bound);
                        b5.assign_intersection(b5, local_row, local_bound);
                        { // full bitmap region

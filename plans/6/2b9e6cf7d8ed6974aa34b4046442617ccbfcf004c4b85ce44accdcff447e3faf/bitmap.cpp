...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b3, false, *this, policy, 3, 0);
    }
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            counter += b3.subtraction_count<bitmap_words>(v4_adj, v4_bit_idx, v4_bit_idx);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            for (size_t v4_idx = 0; v4_idx < s3.size(); v4_idx++) { // loop-4 begin
                ...
                counter += s3.subtract_cnt(v4_adj, v4_adj.vid());
            }
        }
    }
};

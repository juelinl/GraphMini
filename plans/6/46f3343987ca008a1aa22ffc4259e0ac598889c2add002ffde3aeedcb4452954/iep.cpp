    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            for (size_t v2_idx = 0; v2_idx < s2.size(); v2_idx++) { // loop-2 begin
                ...
                /* Val: 1 | Group: (0),(1),(2) | Comp: |VSet(4)|*|VSet(4)|*|VSet(3)| */
                counter += -1ll * s4.intersect_cnt(s3) * s4.size();
                /* Val: -1 | Group: (0 2),(1) | Comp: |VSet(4) & VSet(3)|*|VSet(4)| */
                counter += -1ll * s4.size() * s4.intersect_cnt(s3);
                /* Val: -1 | Group: (0),(1 2) | Comp: |VSet(4)|*|VSet(4) & VSet(3)| */
                counter += -1ll * s4.size() * s3.size();
                /* Val: -1 | Group: (0 1),(2) | Comp: |VSet(4) & VSet(4)|*|VSet(3)| */
                counter += 2ll * s4.intersect(s4).intersect_cnt(s3);
                /* Val: 2 | Group: (0 1 2) | Comp: |VSet(4) & VSet(4) & VSet(3)| */
            }
        }
    }
};
...
void plan(const GraphType *graph, Context &ctx) {
    ...
    const QueryContext query{graph, ctx, progress, {0, nested_threshold(graph->num_vertex, graph->num_edge, graph->max_degree, 4), 0, 0, 0}};

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s5, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s4, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s3, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s1 = v1_adj.bounded(v0_adj.vid());
            if (s1.size() == 0)
                continue;
            VertexSet s2 = s0.bounded(v1_adj.vid());
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) {                  // loop-1 begin
                ...
                for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                    ...
                    VertexSet v2_adj = query.graph->N(v2);
                    VertexSet s3 = s2.intersect(v2_adj);
                    std::optional<Bitmap> bitmap_s3;
                    if (bitmap_rows) {
                        bitmap_s3.emplace(Bitmap::from_sorted(bitmap_rows->universe(), s3.data(), s3.size()));
                    }
                    if (bitmap_rows) { // full bitmap region
                        ...
                        auto bitmap_execute = [&](auto bitmap_tag) {
                            ...
                            return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s3)();
                        };
                        ...
                        for (size_t v3_idx = 0; v3_idx < s3.size(); ++v3_idx) {
                            const IdType v3 = s3[v3_idx];
                            VertexSet v3_adj = query.graph->N(v3);
                            VertexSet s4 = s3.bounded(v3_adj.vid());
                            if (s4.size() > query.nested_thresholds[4]) {

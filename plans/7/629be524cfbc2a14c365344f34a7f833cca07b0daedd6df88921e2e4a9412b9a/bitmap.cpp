...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s9, false, *this, policy, 4, 0);
    }
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s8.assign_intersection<bitmap_words, true>(s3, v3_adj, v3_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s3.assign_bounded<bitmap_words, true>(s1, v2_bit_idx);
            s4.assign_removed<bitmap_words, true>(s2, v2_bit_idx);
            const auto v2_adj = bitgraph.local_row(v2_bit_idx);
            s5.assign_intersection<bitmap_words, true>(s1, v2_adj, v2_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            s2.assign_intersection<bitmap_words, true>(s0, v1_adj, v1_bit_idx);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s7, VertexSet &_s8, VertexSet &_s6) : query{_query}, s7{_s7}, s8{_s8}, s6{_s6} {};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = s3.intersect(v3_adj, v3_adj.vid());
            if (s6.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel4(query, s7, s8, s6), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s4 = s2.remove(v2_adj.vid());
            VertexSet s5 = s1.intersect(v2_adj, v2_adj.vid());

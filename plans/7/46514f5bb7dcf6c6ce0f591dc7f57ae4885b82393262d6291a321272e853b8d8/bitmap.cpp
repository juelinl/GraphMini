...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b5, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b4, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b3, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s1 = v1_adj.subtract(v0_adj, v0_adj.vid());
            if (s1.size() == 0)
                continue;
            VertexSet s2 = s0.subtract(v1_adj, v1_adj.vid());
            ...
            if (bitmap_rows) {                                                       // bitmap-enabled continuation
                ...
                for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) { // loop-1 begin
                    ...
                    for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                        ...
                        VertexSet v2_adj = query.graph->N(v2);
                        VertexSet s3 = s2.intersect(v2_adj);
                        Bitmap b3 = Bitmap::from_sorted(bitgraph.universe(), s3.data(), s3.size());
                        { // full bitmap region
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                constexpr size_t bitmap_words = decltype(bitmap_tag)::value;
                                return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, b3)();
                            };

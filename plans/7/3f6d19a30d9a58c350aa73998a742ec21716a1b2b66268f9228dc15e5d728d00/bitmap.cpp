...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b4, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b7.assign_intersection<bitmap_words, true>(b5, v3_adj);
            b8.assign_subtraction<bitmap_words, true>(b6, v3_adj, v3_bit_idx);
                    ...
                    for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                        ...
                        { // full bitmap region
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, b4, b5, b6)();
                            };

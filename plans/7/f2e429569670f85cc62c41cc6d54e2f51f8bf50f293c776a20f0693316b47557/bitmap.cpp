...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b10, false, *this, policy, 1, 0);
    }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s7 = s4.subtract(v2_adj);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s3 = v1_adj.subtract(v0_adj, v0_adj.vid());
            if (s3.size() == 0)
                continue;
                        ...
                        for (size_t v3_idx = 0; v3_idx < s7.size(); v3_idx++) { // loop-3 begin
                            ...
                            { // full bitmap region
                                auto bitmap_execute = [&](auto bitmap_tag) {
                                    ...
                                    return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, b8, b9)();
                                };

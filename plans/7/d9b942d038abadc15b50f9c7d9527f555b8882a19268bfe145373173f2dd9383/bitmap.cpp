...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s12, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s9, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s10.assign_intersection<bitmap_words, true>(s6, v3_adj, v3_bit_idx);
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s5, VertexSet &_s7, VertexSet &_s6) : query{_query}, s5{_s5}, s7{_s7}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s10 = s6.intersect(v3_adj, v3_adj.vid());
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s5, s7, s6), tbb::auto_partitioner());
                continue;
            }
                ...
                for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                    ...
                    if (bitmap_rows) { // full bitmap region
                        ...
                        auto bitmap_execute = [&](auto bitmap_tag) {
                            ...
                            return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s5, *bitmap_s6, *bitmap_s7)();
                        };

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, false, *this, policy, 1, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b6, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b7.assign_subtraction<bitmap_words, true>(b5, v4_adj, v4_bit_idx);
            b8.assign_subtraction<bitmap_words, true>(b6, v4_adj, v4_bit_idx, v4_bit_idx);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s7 = s5.subtract(v4_adj);
            VertexSet s8 = s6.subtract(v4_adj, v4_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s5 = v3_adj.subtract(v0_adj).subtract(v1_adj).subtract(v2_adj);
            if (s5.size() == 0)
                continue;
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s3 = v2_adj.subtract(v0_adj).subtract(v1_adj);
            if (s3.size() == 0)
                continue;
            VertexSet s4 = s2.subtract(v2_adj, v2_adj.vid());
                        ...
                        if (bitmap_rows) {                                                       // bitmap-enabled continuation
                            ...
                            { // full bitmap region
                                auto bitmap_execute = [&](auto bitmap_tag) {
                                    ...
                                    return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, b5, b6)();
                                };

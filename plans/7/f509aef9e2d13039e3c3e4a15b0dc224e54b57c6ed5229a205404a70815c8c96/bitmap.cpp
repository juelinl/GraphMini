...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s11, false, *this, policy, 1, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s9, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s2, VertexSet &_s4, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s2{_s2}, s4{_s4}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s5 = v2_adj.remove(v0_adj.vid()).remove(v1_adj.vid());
            if (s5.size() == 0)
                continue;
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, v0_adj, v1_adj, s2, s4, s3), tbb::auto_partitioner());
                continue;
            }
                ...
                for (size_t v2_idx = 0; v2_idx < s3.size(); v2_idx++) { // loop-2 begin
                    ...
                    for (size_t v3_idx = 0; v3_idx < s6.size(); v3_idx++) {                  // loop-3 begin
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            for (size_t v4_idx = 0; v4_idx < s9.size(); ++v4_idx) {
                                const IdType v4 = s9[v4_idx];
                                VertexSet v4_adj = query.graph->N(v4);
                                VertexSet s10 = s8.remove(v4_adj.vid());
                                VertexSet s11 = s9.bounded(v4_adj.vid());

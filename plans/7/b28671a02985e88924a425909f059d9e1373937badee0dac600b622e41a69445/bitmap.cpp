...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b10, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b7, const Bitmap &input_b8)
        : query(query), bitgraph(bitgraph), policy(policy), input_b7(input_b7), input_b8(input_b8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b9.assign_subtraction<bitmap_words, true>(b7, v4_adj, v4_bit_idx);
            b10.assign_subtraction<bitmap_words, true>(b8, v4_adj, v4_bit_idx, v4_bit_idx);
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b5, const Bitmap &input_b6)
        : query(query), bitgraph(bitgraph), policy(policy), input_b5(input_b5), input_b6(input_b6) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b6, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b7.assign_subtraction<bitmap_words, true>(b5, v3_adj, v3_bit_idx);
            b8.assign_subtraction<bitmap_words, true>(b6, v3_adj, v3_bit_idx, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b7, b8)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s9 = s7.subtract(v4_adj);
            VertexSet s10 = s8.subtract(v4_adj, v4_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s7 = s5.subtract(v3_adj);
            VertexSet s8 = s6.subtract(v3_adj, v3_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = s4.subtract(v2_adj);
                    ...
                    for (size_t v2_idx = 0; v2_idx < s3.size(); v2_idx++) { // loop-2 begin
                        ...
                        { // full bitmap region
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, b5, b6)();
                            };

...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b6)
        : query(query), bitgraph(bitgraph), policy(policy), input_b6(input_b6) {}
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b6.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx, v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b6.assign_intersection<bitmap_words, true>(b5, v4_adj);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b6)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            b2.assign_intersection<bitmap_words, true>(b0, v1_adj, v1_bit_idx);

...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_v2_adj, VertexSet &_s5, VertexSet &_s4) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, v2_adj{_v2_adj}, s5{_s5}, s4{_s4} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s6 = v3_adj.remove(v0_adj.vid()).remove(v1_adj.vid()).remove(v2_adj.vid());
            if (s6.size() == 0)
                continue;
            ...
            for (size_t v4_idx = 0; v4_idx < s7.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s9.size() * s8.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(9)|*|VSet(8)| */
                counter += -1ll * s9.intersect_cnt(s8);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(9) & VSet(8)| */
            }
        }
    }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s4.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel3(query, v0_adj, v1_adj, v2_adj, s5, s4), tbb::auto_partitioner());
                continue;
            }

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s6, false, *this, policy, 1, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s5, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
                    ...
                    for (size_t v3_idx = 0; v3_idx < s3.size(); v3_idx++) { // loop-3 begin
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s5)();
                            };
                            ...
                            for (size_t v4_idx = 0; v4_idx < s5.size(); ++v4_idx) {
                                const IdType v4 = s5[v4_idx];
                                VertexSet v4_adj = query.graph->N(v4);
                                VertexSet s6 = s5.bounded(v4_adj.vid());
                                for (size_t v5_idx = 0; v5_idx < s6.size(); v5_idx++) { // loop-5 begin

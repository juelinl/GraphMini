...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s7, const Bitmap &input_s8)
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s9.assign_removed<bitmap_words, true>(s7, v4_bit_idx);
            s10.assign_bounded<bitmap_words, true>(s8, v4_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s7.assign_intersection<bitmap_words, true>(s4, v3_adj);
            s8.assign_removed<bitmap_words, true>(s6, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s7, s8)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s6.assign_intersection<bitmap_words, true>(s2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s4, s5, s6)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s7, VertexSet &_s8) : query{_query}, s7{_s7}, s8{_s8} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s9 = s7.remove(v4_adj.vid());
            VertexSet s10 = s8.bounded(v4_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = s6.remove(v3_adj.vid());
            if (s8.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s8.size(), 1), SetLevel4(query, s7, s8), tbb::auto_partitioner());
                continue;
            }

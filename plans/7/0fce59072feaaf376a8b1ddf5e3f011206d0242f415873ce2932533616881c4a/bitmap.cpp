...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b8)
        : query(query), bitgraph(bitgraph), policy(policy), input_b8(input_b8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, false, *this, policy, 2, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b8.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx, v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b7)
        : query(query), bitgraph(bitgraph), policy(policy), input_b7(input_b7) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b8.assign_subtraction<bitmap_words, true>(b7, v4_adj, v4_bit_idx, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b8)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b7.assign_intersection<bitmap_words, true>(b6, v3_adj);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b7)();
        }
        return counter;
    }
};

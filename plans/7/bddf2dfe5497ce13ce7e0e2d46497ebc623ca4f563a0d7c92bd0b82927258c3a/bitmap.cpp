...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s7, const Bitmap &input_s8)
        : query(query), bitgraph(bitgraph), policy(policy), input_s7(input_s7), input_s8(input_s8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s8, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s7.removed_count<bitmap_words>(v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s5, const Bitmap &input_s6)
        : query(query), bitgraph(bitgraph), policy(policy), input_s5(input_s5), input_s6(input_s6) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s6, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s7.assign_removed<bitmap_words, true>(s5, v4_bit_idx);
            s8.assign_bounded<bitmap_words, true>(s6, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s7, s8)();
        }
        return counter;
    }
};
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s4)
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s5.assign_removed<bitmap_words, true>(s4, v3_bit_idx);
            const auto v3_adj = bitgraph.local_row(v3_bit_idx);
            s6.assign_intersection<bitmap_words, true>(s4, v3_adj);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s5, s6)();
        }
        return counter;
    }
};
...
class BitLevel2 {
  ...
  public:
    BitLevel2(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s2, const Bitmap &input_s3)
        : query(query), bitgraph(bitgraph), policy(policy), input_s2(input_s2), input_s3(input_s3) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s3, true, *this, policy, 1, query.nested_thresholds[2]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s4.assign_intersection<bitmap_words, true>(s2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s4)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            s3.assign_intersection<bitmap_words, true>(s1, v1_adj, v1_bit_idx);
            counter += BitLevel2<bitmap_words>(query, bitgraph, policy, s2, s3)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s5, VertexSet &_s6) : query{_query}, s5{_s5}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s7 = s5.remove(v4_adj.vid());
            VertexSet s8 = s6.bounded(v4_adj.vid());
            for (size_t v5_idx = 0; v5_idx < s8.size(); v5_idx++) { // loop-5 begin
                const IdType v5 = s8[v5_idx];
                VertexSet v5_adj = query.graph->N(v5);
                counter += s7.remove_cnt(v5_adj.vid());
            }
        }
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s6 = s4.intersect(v3_adj);
            if (s6.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel4(query, s5, s6), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            auto bitmap_rows = BitGraph::build(*query.graph, v0, v0_adj, v0_adj, 9); // bitmap-region build once per anchor

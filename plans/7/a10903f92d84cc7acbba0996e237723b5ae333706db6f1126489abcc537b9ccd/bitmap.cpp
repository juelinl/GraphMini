...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b9)
        : query(query), bitgraph(bitgraph), policy(policy), input_b9(input_b9) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b9, false, *this, policy, 1, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b9.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx, v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b9)();
        }
        return counter;
    }
};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            for (size_t v5_idx = 0; v5_idx < s9.size(); v5_idx++) { // loop-5 begin
                ...
                counter += s9.subtract_cnt(v5_adj, v5_adj.vid());
            }
        }
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = s6.subtract(v3_adj);
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s2, VertexSet &_s3, VertexSet &_s1) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s2{_s2}, s3{_s3}, s1{_s1} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s4 = v2_adj.subtract(v0_adj).subtract(v1_adj);
            if (s4.size() == 0)
                continue;
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s1.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s1.size(), 1), SetLevel2(query, v0_adj, v1_adj, s2, s3, s1), tbb::auto_partitioner());
                continue;
            }
                        ...
                        for (size_t v3_idx = 0; v3_idx < s4.size(); v3_idx++) { // loop-3 begin
                            ...
                            { // full bitmap region
                                auto bitmap_execute = [&](auto bitmap_tag) {
                                    ...
                                    return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, b7, b8)();
                                };

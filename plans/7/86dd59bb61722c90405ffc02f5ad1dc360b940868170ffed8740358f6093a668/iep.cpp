...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s6, VertexSet &_s8, VertexSet &_s9, VertexSet &_s7) : query{_query}, s6{_s6}, s8{_s8}, s9{_s9}, s7{_s7} {};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s7.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel3(query, s6, s8, s9, s7), tbb::auto_partitioner());
                continue;
            }

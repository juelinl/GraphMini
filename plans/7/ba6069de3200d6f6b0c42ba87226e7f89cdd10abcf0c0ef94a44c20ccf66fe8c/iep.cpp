    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            for (size_t v3_idx = 0; v3_idx < s3.size(); v3_idx++) { // loop-3 begin
                ...
                VertexSet s6 = s3.bounded(v3_adj.vid());
                VertexSet s7 = s4.remove(v3_adj.vid());
                VertexSet s8 = s5.remove(v3_adj.vid());
                counter += 1ll * s6.size() * s8.size() * s7.size();
                /* Val: 1 | Group: (0),(1),(2) | Comp: |VSet(6)|*|VSet(8)|*|VSet(7)| */
                counter += -1ll * s6.intersect_cnt(s7) * s8.size();
                /* Val: -1 | Group: (0 2),(1) | Comp: |VSet(6) & VSet(7)|*|VSet(8)| */
                counter += -1ll * s6.size() * s8.intersect_cnt(s7);
                /* Val: -1 | Group: (0),(1 2) | Comp: |VSet(6)|*|VSet(8) & VSet(7)| */
                counter += -1ll * s6.intersect_cnt(s8) * s7.size();
                /* Val: -1 | Group: (0 1),(2) | Comp: |VSet(6) & VSet(8)|*|VSet(7)| */
                counter += 2ll * s6.intersect(s8).intersect_cnt(s7);
                /* Val: 2 | Group: (0 1 2) | Comp: |VSet(6) & VSet(8) & VSet(7)| */
            }
        }
    }
};

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s13, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s11, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s5, const Bitmap &input_s6, const Bitmap &input_s7, const Bitmap &input_s8)
        : query(query), bitgraph(bitgraph), policy(policy), input_s5(input_s5), input_s6(input_s6), input_s7(input_s7), input_s8(input_s8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s8, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s11.assign_removed<bitmap_words, true>(s7, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s9, s10, s11)();
        }
        return counter;
    }
};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s5, VertexSet &_s6, VertexSet &_s7, VertexSet &_s8) : query{_query}, s5{_s5}, s6{_s6}, s7{_s7}, s8{_s8} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s11 = s7.remove(v3_adj.vid());
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s3, VertexSet &_s4, VertexSet &_s1) : query{_query}, s2{_s2}, s3{_s3}, s4{_s4}, s1{_s1} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s8.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s8.size(), 1), SetLevel3(query, s5, s6, s7, s8), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s1.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s1.size(), 1), SetLevel2(query, s2, s3, s4, s1), tbb::auto_partitioner());
                continue;
            }
                ...
                for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                    ...
                    if (bitmap_rows) { // full bitmap region
                        ...
                        auto bitmap_execute = [&](auto bitmap_tag) {
                            ...
                            return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s5, *bitmap_s6, *bitmap_s7, *bitmap_s8)();
                        };
                        ...
                        for (size_t v3_idx = 0; v3_idx < s8.size(); ++v3_idx) {
                            const IdType v3 = s8[v3_idx];
                            VertexSet v3_adj = query.graph->N(v3);
                            VertexSet s9 = s5.remove(v3_adj.vid());
                            VertexSet s10 = s6.intersect(v3_adj);

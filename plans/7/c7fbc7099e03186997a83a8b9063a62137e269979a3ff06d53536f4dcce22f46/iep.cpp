...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s6, VertexSet &_s7, VertexSet &_s8, VertexSet &_s9) : query{_query}, s6{_s6}, s7{_s7}, s8{_s8}, s9{_s9} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s12 = s8.intersect(v3_adj);
            for (size_t v4_idx = 0; v4_idx < s12.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s14.size() * s13.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(14)|*|VSet(13)| */
                counter += -1ll * s14.intersect_cnt(s13);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(14) & VSet(13)| */
            }
        }
    }
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s2, VertexSet &_s4, VertexSet &_s5, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s2{_s2}, s4{_s4}, s5{_s5}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s9.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s9.size(), 1), SetLevel3(query, s6, s7, s8, s9), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, v0_adj, v1_adj, s2, s4, s5, s3), tbb::auto_partitioner());
                continue;
            }

...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s5)
        : query(query), bitgraph(bitgraph), policy(policy), input_s5(input_s5) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s5, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s5.bounded_count<bitmap_words>(v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s4)
        : query(query), bitgraph(bitgraph), policy(policy), input_s4(input_s4) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s4, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s5.assign_bounded<bitmap_words, true>(s4, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s5)();
        }
        return counter;
    }
};
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s3)
        : query(query), bitgraph(bitgraph), policy(policy), input_s3(input_s3) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s3, true, *this, policy, 2, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s4.assign_bounded<bitmap_words, true>(s3, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s4)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s3.assign_bounded<bitmap_words, true>(s2, v2_bit_idx);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s3)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s4) : query{_query}, s4{_s4} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            const IdType v4 = s4[v4_idx];
            VertexSet v4_adj = query.graph->N(v4);
            VertexSet s5 = s4.bounded(v4_adj.vid());
            for (size_t v5_idx = 0; v5_idx < s5.size(); v5_idx++) { // loop-5 begin
                const IdType v5 = s5[v5_idx];
                VertexSet v5_adj = query.graph->N(v5);
                counter += s5.bounded_cnt(v5_adj.vid());
            }
        }
    }
};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s3) : query{_query}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s4 = s3.bounded(v3_adj.vid());
            if (s4.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel4(query, s4), tbb::auto_partitioner());
                continue;
            }
            for (size_t v4_idx = 0; v4_idx < s4.size(); v4_idx++) { // loop-4 begin
                const IdType v4 = s4[v4_idx];
                VertexSet v4_adj = query.graph->N(v4);
                VertexSet s5 = s4.bounded(v4_adj.vid());
                for (size_t v5_idx = 0; v5_idx < s5.size(); v5_idx++) { // loop-5 begin
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s3 = s2.bounded(v2_adj.vid());
            if (s3.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel3(query, s3), tbb::auto_partitioner());
                continue;
            }

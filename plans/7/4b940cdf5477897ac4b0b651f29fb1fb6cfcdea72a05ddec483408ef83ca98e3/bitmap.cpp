...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b6, const Bitmap &input_b7)
        : query(query), bitgraph(bitgraph), policy(policy), input_b6(input_b6), input_b7(input_b7) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b6, false, *this, policy, 4, 0);
    }
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b4, const Bitmap &input_b5)
        : query(query), bitgraph(bitgraph), policy(policy), input_b4(input_b4), input_b5(input_b5) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b4, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b6.assign_subtraction<bitmap_words, true>(b5, v4_adj, v4_bit_idx);
            b7.assign_intersection<bitmap_words, true>(b4, v4_adj, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b6, b7)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b4.assign_subtraction<bitmap_words, true>(b3, v3_adj, v3_bit_idx, v3_bit_idx);
            b5.assign_intersection<bitmap_words, true>(b3, v3_adj, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b4, b5)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s5, VertexSet &_s4) : query{_query}, s5{_s5}, s4{_s4} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s6 = s5.subtract(v4_adj);
            VertexSet s7 = s4.intersect(v4_adj, v4_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s4 = s3.subtract(v3_adj, v3_adj.vid());
            VertexSet s5 = s3.intersect(v3_adj, v3_adj.vid());
            if (s4.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel4(query, s5, s4), tbb::auto_partitioner());
                continue;
            }

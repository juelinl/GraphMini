...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s6, true, *this, policy, 2, query.nested_thresholds[3]);
    }
    template <class Cursor>
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s4.assign_removed<bitmap_words, true>(s1, v2_bit_idx);
            const auto v2_adj = bitgraph.local_row(v2_bit_idx);
            s5.assign_intersection<bitmap_words, true>(s2, v2_adj);
            s6.assign_intersection<bitmap_words, true>(s3, v2_adj, v2_bit_idx);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = s3.intersect(v2_adj, v2_adj.vid());

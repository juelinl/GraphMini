...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s7)
        : query(query), bitgraph(bitgraph), policy(policy), input_s7(input_s7) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s7, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s7.bounded_count<bitmap_words>(v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s7)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s6.assign_intersection<bitmap_words, true>(s4, v3_adj, v3_bit_idx);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            for (size_t v5_idx = 0; v5_idx < s7.size(); v5_idx++) { // loop-5 begin
                const IdType v5 = s7[v5_idx];
                VertexSet v5_adj = query.graph->N(v5);
                counter += s7.bounded_cnt(v5_adj.vid());
            }
        }
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            auto bitmap_rows = BitGraph::build(*query.graph, v0, v0_adj, v0_adj, 8); // bitmap-region build once per anchor

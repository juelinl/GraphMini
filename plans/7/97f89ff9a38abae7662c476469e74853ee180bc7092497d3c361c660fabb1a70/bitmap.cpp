...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s9)
        : query(query), bitgraph(bitgraph), policy(policy), input_s9(input_s9) {}
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s9.bounded_count<bitmap_words>(v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s9)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s6.assign_intersection<bitmap_words, true>(s2, v2_adj, v2_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            s3.assign_intersection<bitmap_words, true>(s0, v1_adj, v1_bit_idx);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            for (size_t v5_idx = 0; v5_idx < s9.size(); v5_idx++) { // loop-5 begin
                ...
                counter += s9.bounded_cnt(v5_adj.vid());
            }
        }
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = s2.intersect(v2_adj, v2_adj.vid());

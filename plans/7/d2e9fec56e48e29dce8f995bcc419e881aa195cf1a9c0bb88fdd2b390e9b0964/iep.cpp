...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s7, VertexSet &_s8, VertexSet &_s9, VertexSet &_s10) : query{_query}, s7{_s7}, s8{_s8}, s9{_s9}, s10{_s10} {};
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s3, VertexSet &_s4, VertexSet &_s6, VertexSet &_s5) : query{_query}, s2{_s2}, s3{_s3}, s4{_s4}, s6{_s6}, s5{_s5} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s10.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s10.size(), 1), SetLevel3(query, s7, s8, s9, s10), tbb::auto_partitioner());
                continue;
            }
            for (size_t v3_idx = 0; v3_idx < s10.size(); v3_idx++) { // loop-3 begin
                const IdType v3 = s10[v3_idx];
                VertexSet v3_adj = query.graph->N(v3);
                VertexSet s11 = s7.intersect(v3_adj);
                VertexSet s12 = s8.remove(v3_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s6 = s5.bounded(v1);
            if (s5.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s5.size(), 1), SetLevel2(query, s2, s3, s4, s6, s5), tbb::auto_partitioner());
                continue;
            }

...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s7, VertexSet &_s5, VertexSet &_s6, VertexSet &_s8) : query{_query}, s7{_s7}, s5{_s5}, s6{_s6}, s8{_s8} {};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s8 = s7.bounded(v2);
            if (s8.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s8.size(), 1), SetLevel3(query, s7, s5, s6, s8), tbb::auto_partitioner());
                continue;
            }

...
class BitLevel2 {
  ...
  public:
    BitLevel2(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s1, const Bitmap &input_s2, const Bitmap &input_s3)
        : query(query), bitgraph(bitgraph), policy(policy), input_s1(input_s1), input_s2(input_s2), input_s3(input_s3) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s4.assign_intersection<bitmap_words, true>(s1, v2_adj);
            s5.assign_removed<bitmap_words, true>(s2, v2_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            s3.assign_bounded<bitmap_words, true>(s2, v1_bit_idx);
            counter += BitLevel2<bitmap_words>(query, bitgraph, policy, s1, s2, s3)();
        }
        return counter;
    }
};
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s1, VertexSet &_s2, VertexSet &_s3) : query{_query}, s1{_s1}, s2{_s2}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, s1, s2, s3), tbb::auto_partitioner());
                continue;
            }

...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b4)
        : query(query), bitgraph(bitgraph), policy(policy), input_b4(input_b4) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b4, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b4.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx, v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b3)
        : query(query), bitgraph(bitgraph), policy(policy), input_b3(input_b3) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b3, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b4.assign_subtraction<bitmap_words, true>(b3, v4_adj, v4_bit_idx, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b4)();
        }
        return counter;
    }
};
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b2)
        : query(query), bitgraph(bitgraph), policy(policy), input_b2(input_b2) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b2, true, *this, policy, 2, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b3.assign_subtraction<bitmap_words, true>(b2, v3_adj, v3_bit_idx, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b3)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b2.assign_subtraction<bitmap_words, true>(b1, v2_adj, v2_bit_idx, v2_bit_idx);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, b2)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s3 = s2.subtract(v3_adj, v3_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s2 = s1.subtract(v2_adj, v2_adj.vid());

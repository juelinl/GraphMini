...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_v2_adj, VertexSet &_s4) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, v2_adj{_v2_adj}, s4{_s4} {};
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s2, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s2{_s2}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s4.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel3(query, v0_adj, v1_adj, v2_adj, s4), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, v0_adj, v1_adj, s2, s3), tbb::auto_partitioner());
                continue;
            }

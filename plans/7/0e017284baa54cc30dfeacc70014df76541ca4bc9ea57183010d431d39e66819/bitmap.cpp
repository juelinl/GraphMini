...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b7, const Bitmap &input_b8)
        : query(query), bitgraph(bitgraph), policy(policy), input_b7(input_b7), input_b8(input_b8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b7.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b4, const Bitmap &input_b5, const Bitmap &input_b6)
        : query(query), bitgraph(bitgraph), policy(policy), input_b4(input_b4), input_b5(input_b5), input_b6(input_b6) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b6, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b7.assign_intersection<bitmap_words, true>(b4, v4_adj);
            b8.assign_subtraction<bitmap_words, true>(b5, v4_adj, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, b7, b8)();
        }
        return counter;
    }
};
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b3)
        : query(query), bitgraph(bitgraph), policy(policy), input_b3(input_b3) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b3, true, *this, policy, 2, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b4.assign_subtraction<bitmap_words, true>(b3, v3_adj, v3_bit_idx);
            b5.assign_intersection<bitmap_words, true>(b3, v3_adj);
            b6.assign_bounded<bitmap_words, true>(b5, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b4, b5, b6)();
        }
        return counter;
    }
};
...
class BitLevel2 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b1, true, *this, policy, 1, query.nested_thresholds[2]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b3.assign_intersection<bitmap_words, true>(b2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, b3)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            b1.assign_subtraction<bitmap_words, true>(b0, v1_adj, v1_bit_idx, v1_bit_idx);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s4, VertexSet &_s5, VertexSet &_s6) : query{_query}, s4{_s4}, s5{_s5}, s6{_s6} {};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s4 = s3.subtract(v3_adj);
            VertexSet s5 = s3.intersect(v3_adj);
            VertexSet s6 = s5.bounded(v3);
            if (s6.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel4(query, s4, s5, s6), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s3 = s2.intersect(v2_adj);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s1 = s0.subtract(v1_adj, v1_adj.vid());

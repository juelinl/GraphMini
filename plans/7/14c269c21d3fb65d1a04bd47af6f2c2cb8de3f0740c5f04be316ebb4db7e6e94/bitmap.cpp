...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s8)
        : query(query), bitgraph(bitgraph), policy(policy), input_s8(input_s8) {}
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s8.bounded_count<bitmap_words>(v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s8.assign_removed<bitmap_words, true>(s7, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s8)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s7, VertexSet &_s6) : query{_query}, s7{_s7}, s6{_s6} {};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            if (s6.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel4(query, s7, s6), tbb::auto_partitioner());
                continue;
            }

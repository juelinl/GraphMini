...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s3, const Bitmap &input_s4, const Bitmap &input_s5, const Bitmap &input_s6)
        : query(query), bitgraph(bitgraph), policy(policy), input_s3(input_s3), input_s4(input_s4), input_s5(input_s5), input_s6(input_s6) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s7.assign_removed<bitmap_words, true>(s3, v3_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s6.assign_bounded<bitmap_words, true>(s5, v2_bit_idx);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s3, s4, s5, s6)();
        }
        return counter;
    }
};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s3, VertexSet &_s4, VertexSet &_s5, VertexSet &_s6) : query{_query}, s3{_s3}, s4{_s4}, s5{_s5}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s7 = s3.remove(v3_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = s5.bounded(v2);
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s3, s4, s5, s6), tbb::auto_partitioner());
                continue;
            }

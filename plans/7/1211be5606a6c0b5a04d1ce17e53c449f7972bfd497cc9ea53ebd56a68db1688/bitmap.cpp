...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s6, VertexSet &_s5, VertexSet &_s4) : query{_query}, s6{_s6}, s5{_s5}, s4{_s4} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s7 = s6.remove(v3_adj.vid());
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s1, VertexSet &_s3) : query{_query}, s2{_s2}, s1{_s1}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s4.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel3(query, s6, s5, s4), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, s2, s1, s3), tbb::auto_partitioner());
                continue;
            }
                    ...
                    for (size_t v3_idx = 0; v3_idx < s4.size(); v3_idx++) {                  // loop-3 begin
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s7, *bitmap_s8, *bitmap_s9)();
                            };

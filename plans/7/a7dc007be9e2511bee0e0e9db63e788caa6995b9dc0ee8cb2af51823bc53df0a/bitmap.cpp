...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b9, false, *this, policy, 4, 0);
    }
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b8.assign_intersection<bitmap_words, true>(b5, v4_adj);
            b9.assign_removed<bitmap_words, true>(b7, v4_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b6.assign_removed<bitmap_words, true>(b4, v3_bit_idx);
            const auto v3_adj = bitgraph.local_row(v3_bit_idx);
            b7.assign_intersection<bitmap_words, true>(b4, v3_adj, v3_bit_idx);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s5, VertexSet &_s7, VertexSet &_s6) : query{_query}, s5{_s5}, s7{_s7}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s8 = s5.intersect(v4_adj);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            if (s6.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel4(query, s5, s7, s6), tbb::auto_partitioner());
                continue;
            }

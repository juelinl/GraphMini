...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s5, VertexSet &_s7, VertexSet &_s8, VertexSet &_s6) : query{_query}, s5{_s5}, s7{_s7}, s8{_s8}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            for (size_t v4_idx = 0; v4_idx < s11.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s13.size() * s12.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(13)|*|VSet(12)| */
                counter += -1ll * s13.intersect_cnt(s12);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(13) & VSet(12)| */
            }
        }
    }
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s1, VertexSet &_s2, VertexSet &_s4, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s1{_s1}, s2{_s2}, s4{_s4}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s5.size() == 0)
                continue;
            VertexSet s6 = s1.remove(v2_adj.vid());
            VertexSet s7 = s2.intersect(v2_adj);
            VertexSet s8 = s4.remove(v2_adj.vid());
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s5, s7, s8, s6), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, v0_adj, v1_adj, s1, s2, s4, s3), tbb::auto_partitioner());
                continue;
            }

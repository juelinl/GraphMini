...
class BitLevel5 {
    ...
    const Bitmap &input_s14;

  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s13, const Bitmap &input_s14)
        : query(query), bitgraph(bitgraph), policy(policy), input_s13(input_s13), input_s14(input_s14) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s14, false, *this, policy, 1, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        BitmapTaskInput task_s14(input_s14, parallel_task, policy);
        const Bitmap &s14 = task_s14.get();
        uint64_t counter = 0;
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            const auto v5_bit_idx = bc5.position();
            const uint64_t previous_count = counter;
            counter += s13.removed_count<bitmap_words>(v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s10, const Bitmap &input_s11, const Bitmap &input_s12)
        : query(query), bitgraph(bitgraph), policy(policy), input_s10(input_s10), input_s11(input_s11), input_s12(input_s12) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s12, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s14.assign_removed<bitmap_words, true>(s11, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s13, s14)();
        }
        return counter;
    }
};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            for (size_t v5_idx = 0; v5_idx < s14.size(); v5_idx++) { // loop-5 begin
                ...
                counter += s13.remove_cnt(v5_adj.vid());
            }
        }
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = v2_adj.remove(v0_adj.vid()).remove(v1_adj.vid());
            if (s6.size() == 0)
                continue;
            VertexSet s7 = s3.intersect(v2_adj);
            VertexSet s8 = s5.remove(v2_adj.vid());
            ...
            for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) { // loop-1 begin
                ...
                for (size_t v2_idx = 0; v2_idx < s2.size(); v2_idx++) { // loop-2 begin
                    ...
                    for (size_t v3_idx = 0; v3_idx < s8.size(); v3_idx++) {                  // loop-3 begin
                        ...
                        VertexSet s11 = s7.remove(v3_adj.vid());
                        VertexSet s12 = s9.intersect(v3_adj);
                        std::optional<Bitmap> bitmap_s12;
                        std::optional<Bitmap> bitmap_s10;
                        std::optional<Bitmap> bitmap_s11;
                        if (bitmap_rows) {
                            bitmap_s12.emplace(Bitmap::from_sorted(bitmap_rows->universe(), s12.data(), s12.size()));
                        }
                        if (bitmap_rows) {
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s10, *bitmap_s11, *bitmap_s12)();
                            };
                            ...
                            for (size_t v4_idx = 0; v4_idx < s12.size(); ++v4_idx) {
                                const IdType v4 = s12[v4_idx];
                                VertexSet v4_adj = query.graph->N(v4);
                                VertexSet s13 = s10.remove(v4_adj.vid());
                                VertexSet s14 = s11.remove(v4_adj.vid());

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s10, false, *this, policy, 2, 0);
    }
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s5, const Bitmap &input_s6)
        : query(query), bitgraph(bitgraph), policy(policy), input_s5(input_s5), input_s6(input_s6) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s6, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s8.assign_bounded<bitmap_words, true>(s6, v3_bit_idx);
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s5, VertexSet &_s6) : query{_query}, s5{_s5}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = s6.bounded(v3_adj.vid());
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s4, VertexSet &_s3) : query{_query}, s2{_s2}, s4{_s4}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s6 = s4.remove(v2_adj.vid());
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s5, s6), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, s2, s4, s3), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            for (size_t v1_idx = 0; v1_idx < s1.size(); v1_idx++) { // loop-1 begin
                ...
                VertexSet s3 = s0.remove(v1_adj.vid());
                VertexSet s4 = s0.intersect(v1_adj);
                auto bitmap_rows = BitGraph::build(*query.graph, v1, v1_adj, v1_adj, 6); // bitmap-region build once per anchor
                for (size_t v2_idx = 0; v2_idx < s3.size(); v2_idx++) {                  // loop-2 begin
                    const IdType v2 = s3[v2_idx];
                    VertexSet v2_adj = query.graph->N(v2);
                    VertexSet s5 = s2.intersect(v2_adj);
                    ...
                    if (bitmap_rows) { // full bitmap region
                        ...
                        auto bitmap_execute = [&](auto bitmap_tag) {
                            ...
                            return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s5, *bitmap_s6)();
                        };
                        ...
                        for (size_t v3_idx = 0; v3_idx < s6.size(); ++v3_idx) {
                            const IdType v3 = s6[v3_idx];
                            VertexSet v3_adj = query.graph->N(v3);
                            VertexSet s7 = s5.remove(v3_adj.vid());
                            VertexSet s8 = s6.bounded(v3_adj.vid());

...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s6, VertexSet &_s5, VertexSet &_s7, VertexSet &_s4) : query{_query}, s6{_s6}, s5{_s5}, s7{_s7}, s4{_s4} {};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s4.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel3(query, s6, s5, s7, s4), tbb::auto_partitioner());
                continue;
            }

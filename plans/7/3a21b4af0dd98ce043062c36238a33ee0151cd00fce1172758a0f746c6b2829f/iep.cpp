...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s7, VertexSet &_s5, VertexSet &_s8, VertexSet &_s6) : query{_query}, s7{_s7}, s5{_s5}, s8{_s8}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            for (size_t v4_idx = 0; v4_idx < s11.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s12.size() * s13.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(12)|*|VSet(13)| */
                counter += -1ll * s12.intersect_cnt(s13);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(12) & VSet(13)| */
            }
        }
    }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s7, s5, s8, s6), tbb::auto_partitioner());
                continue;
            }

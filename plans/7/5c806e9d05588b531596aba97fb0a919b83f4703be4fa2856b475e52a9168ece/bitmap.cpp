...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b12, false, *this, policy, 4, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b10, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b4, const Bitmap &input_b5, const Bitmap &input_b6, const Bitmap &input_b7)
        : query(query), bitgraph(bitgraph), policy(policy), input_b4(input_b4), input_b5(input_b5), input_b6(input_b6), input_b7(input_b7) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b7, true, *this, policy, 2, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b8.assign_subtraction<bitmap_words, true>(b4, v3_adj, v3_bit_idx);
            b9.assign_subtraction<bitmap_words, true>(b5, v3_adj, v3_bit_idx);
            b10.assign_subtraction<bitmap_words, true>(b6, v3_adj, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b8, b9, b10)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b4.assign_subtraction<bitmap_words, true>(b1, v2_adj, v2_bit_idx);
            b5.assign_intersection<bitmap_words, true>(b1, v2_adj);
            b6.assign_subtraction<bitmap_words, true>(b2, v2_adj, v2_bit_idx);
            b7.assign_intersection<bitmap_words, true>(b2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, b4, b5, b6, b7)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            b3.assign_bounded<bitmap_words, true>(b2, v1_bit_idx);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = s4.subtract(v3_adj);
            VertexSet s9 = s5.subtract(v3_adj);

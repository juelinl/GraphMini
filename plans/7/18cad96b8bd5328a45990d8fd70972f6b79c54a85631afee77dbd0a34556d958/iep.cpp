...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_v2_adj, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, v2_adj{_v2_adj}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s4 = v3_adj.remove(v0_adj.vid()).remove(v1_adj.vid()).remove(v2_adj.vid());
            if (s4.size() == 0)
                continue;
            ...
            for (size_t v4_idx = 0; v4_idx < s5.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s7.size() * s6.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(7)|*|VSet(6)| */
                counter += -1ll * s7.intersect_cnt(s6);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(7) & VSet(6)| */
            }
        }
    }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s3.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel3(query, v0_adj, v1_adj, v2_adj, s3), tbb::auto_partitioner());
                continue;
            }

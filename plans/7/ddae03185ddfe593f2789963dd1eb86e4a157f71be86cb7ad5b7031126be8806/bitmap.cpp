...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s9, const Bitmap &input_s10)
        : query(query), bitgraph(bitgraph), policy(policy), input_s9(input_s9), input_s10(input_s10) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s10, false, *this, policy, 4, 0);
    }
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s9.removed_count<bitmap_words>(v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s6, const Bitmap &input_s7, const Bitmap &input_s8)
        : query(query), bitgraph(bitgraph), policy(policy), input_s6(input_s6), input_s7(input_s7), input_s8(input_s8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s7, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s9.assign_removed<bitmap_words, true>(s6, v4_bit_idx);
            s10.assign_removed<bitmap_words, true>(s8, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s9, s10)();
        }
        return counter;
    }
};
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s3, const Bitmap &input_s4, const Bitmap &input_s5)
        : query(query), bitgraph(bitgraph), policy(policy), input_s3(input_s3), input_s4(input_s4), input_s5(input_s5) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s6.assign_removed<bitmap_words, true>(s3, v3_bit_idx);
            s7.assign_removed<bitmap_words, true>(s5, v3_bit_idx);
            const auto v3_adj = bitgraph.local_row(v3_bit_idx);
            s8.assign_intersection<bitmap_words, true>(s4, v3_adj, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s6, s7, s8)();
        }
        return counter;
    }
};
...
class BitLevel2 {
  ...
  public:
    BitLevel2(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s2)
        : query(query), bitgraph(bitgraph), policy(policy), input_s2(input_s2) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s2, true, *this, policy, 1, query.nested_thresholds[2]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s4.assign_bounded<bitmap_words, true>(s3, v2_bit_idx);
            const auto v2_adj = bitgraph.local_row(v2_bit_idx);
            s5.assign_intersection<bitmap_words, true>(s2, v2_adj, v2_bit_idx);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s3, s4, s5)();
        }
        return counter;
    }
};
...
class BitLevel1 {
  ...
  public:
    BitLevel1(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s0, const Bitmap &input_s1)
        : query(query), bitgraph(bitgraph), policy(policy), input_s0(input_s0), input_s1(input_s1) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_s1, true, *this, policy, 0, query.nested_thresholds[1]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc1, bool parallel_task) const {
        ...
        for (; bc1.valid(); bc1.advance()) { // bitmap local-index loop
            ...
            counter += BitLevel2<bitmap_words>(query, bitgraph, policy, s2)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s6, VertexSet &_s8, VertexSet &_s7) : query{_query}, s6{_s6}, s8{_s8}, s7{_s7} {};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s3, VertexSet &_s5, VertexSet &_s4) : query{_query}, s3{_s3}, s5{_s5}, s4{_s4} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s6 = s3.remove(v3_adj.vid());
            VertexSet s7 = s5.remove(v3_adj.vid());
            VertexSet s8 = s4.intersect(v3_adj, v3_adj.vid());
            if (s7.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel4(query, s6, s8, s7), tbb::auto_partitioner());
                continue;
            }
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2) : query{_query}, s2{_s2} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s4 = s3.bounded(v2);
            VertexSet s5 = s2.intersect(v2_adj, v2_adj.vid());
            if (s4.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s4.size(), 1), SetLevel3(query, s3, s5, s4), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s2.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s2.size(), 1), SetLevel2(query, s2), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            std::optional<Bitmap> bitmap_s1;
            std::optional<Bitmap> bitmap_s0;
            if (bitmap_rows) {
                bitmap_s1.emplace(Bitmap::from_sorted(bitmap_rows->universe(), s1.data(), s1.size()));
            }
            if (bitmap_rows) {
            ...
            if (bitmap_rows) { // full bitmap region
                ...
                auto bitmap_execute = [&](auto bitmap_tag) {
                    ...
                    return BitLevel1<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s0, *bitmap_s1)();
                };
                ...
                for (size_t v1_idx = 0; v1_idx < s1.size(); ++v1_idx) {
                    const IdType v1 = s1[v1_idx];
                    VertexSet v1_adj = query.graph->N(v1);
                    VertexSet s2 = s0.intersect(v1_adj);
                    if (s2.size() > query.nested_thresholds[2]) {

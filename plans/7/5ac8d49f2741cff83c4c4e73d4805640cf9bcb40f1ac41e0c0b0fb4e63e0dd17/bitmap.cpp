...
class BitLevel5 {
  ...
  public:
    BitLevel5(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s6, const Bitmap &input_s7)
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += s6.removed_count<bitmap_words>(v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s4, const Bitmap &input_s5)
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s6.assign_removed<bitmap_words, true>(s4, v4_bit_idx);
            s7.assign_bounded<bitmap_words, true>(s5, v4_bit_idx);
            counter += BitLevel5<bitmap_words>(query, bitgraph, policy, s6, s7)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s4.assign_removed<bitmap_words, true>(s3, v3_bit_idx);
            const auto v3_adj = bitgraph.local_row(v3_bit_idx);
            s5.assign_intersection<bitmap_words, true>(s3, v3_adj);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s4, s5)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s3.assign_intersection<bitmap_words, true>(s2, v2_adj);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s7 = s5.bounded(v4_adj.vid());

...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b5, const Bitmap &input_b6, const Bitmap &input_b7)
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b8.assign_subtraction<bitmap_words, true>(b7, v4_adj, v4_bit_idx);
            b9.assign_intersection<bitmap_words, true>(b6, v4_adj);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b7.assign_intersection<bitmap_words, true>(b4, v3_adj);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b5, b6, b7)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b4.assign_intersection<bitmap_words, true>(b2, v2_adj, v2_bit_idx);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s7, VertexSet &_s6, VertexSet &_s5) : query{_query}, s7{_s7}, s6{_s6}, s5{_s5} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s8 = s7.subtract(v4_adj);
            VertexSet s9 = s6.intersect(v4_adj);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            if (s5.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s5.size(), 1), SetLevel4(query, s7, s6, s5), tbb::auto_partitioner());
                continue;
            }

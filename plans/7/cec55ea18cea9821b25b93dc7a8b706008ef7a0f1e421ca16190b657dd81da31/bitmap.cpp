  ...
  public:
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b14.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b13.assign_subtraction<bitmap_words, true>(b12, v4_adj, v4_bit_idx);
            b14.assign_intersection<bitmap_words, true>(b10, v4_adj);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s12, VertexSet &_s10, VertexSet &_s11) : query{_query}, s12{_s12}, s10{_s10}, s11{_s11} {};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s12 = s9.subtract(v3_adj);
            if (s11.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s11.size(), 1), SetLevel4(query, s12, s10, s11), tbb::auto_partitioner());
                continue;
            }
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s3, VertexSet &_s4, VertexSet &_s5) : query{_query}, s2{_s2}, s3{_s3}, s4{_s4}, s5{_s5} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            const IdType v2 = s5[v2_idx];
            VertexSet v2_adj = query.graph->N(v2);
            VertexSet s6 = s2.intersect(v2_adj);
            VertexSet s7 = s3.intersect(v2_adj);
            VertexSet s8 = s4.subtract(v2_adj);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s5 = s1.intersect(v1_adj, v1_adj.vid());
            if (s5.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s5.size(), 1), SetLevel2(query, s2, s3, s4, s5), tbb::auto_partitioner());
                continue;
            }
            for (size_t v2_idx = 0; v2_idx < s5.size(); v2_idx++) { // loop-2 begin
                const IdType v2 = s5[v2_idx];
                VertexSet v2_adj = query.graph->N(v2);
                VertexSet s6 = s2.intersect(v2_adj);
                VertexSet s7 = s3.intersect(v2_adj);

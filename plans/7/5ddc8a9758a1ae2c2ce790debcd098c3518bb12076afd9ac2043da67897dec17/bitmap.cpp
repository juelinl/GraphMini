...
class BitLevel4 {
  ...
  public:
    BitLevel4(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s5, const Bitmap &input_s6, const Bitmap &input_s7)
        : query(query), bitgraph(bitgraph), policy(policy), input_s5(input_s5), input_s6(input_s6), input_s7(input_s7) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            s8.assign_removed<bitmap_words, true>(s5, v4_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s7.assign_intersection<bitmap_words, true>(s4, v3_adj, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s5, s6, s7)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s5, VertexSet &_s6, VertexSet &_s7) : query{_query}, s5{_s5}, s6{_s6}, s7{_s7} {};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s7 = s4.intersect(v3_adj, v3_adj.vid());
            if (s7.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel4(query, s5, s6, s7), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            auto bitmap_rows = BitGraph::build(*query.graph, v0, v0_adj, v0_adj, 10); // bitmap-region build once per anchor

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b13, false, *this, policy, 2, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b11, true, *this, policy, 1, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b5, const Bitmap &input_b6, const Bitmap &input_b7, const Bitmap &input_b8)
        : query(query), bitgraph(bitgraph), policy(policy), input_b5(input_b5), input_b6(input_b6), input_b7(input_b7), input_b8(input_b8) {}
    uint64_t operator()() const {
        return bitmap_for_each(input_b8, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b11.assign_subtraction<bitmap_words, true>(b7, v3_adj, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, b9, b10, b11)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s11 = s7.subtract(v3_adj);
                    ...
                    for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                        ...
                        { // full bitmap region
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel3<bitmap_words>(query, bitgraph, bitmap_task_policy, b5, b6, b7, b8)();
                            };

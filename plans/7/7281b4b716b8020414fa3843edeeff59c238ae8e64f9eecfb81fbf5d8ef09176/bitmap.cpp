...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s7, true, *this, policy, 0, query.nested_thresholds[3]);
    }
    template <class Cursor>
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s9, VertexSet &_s8) : query{_query}, s9{_s9}, s8{_s8} {};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            if (s8.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s8.size(), 1), SetLevel4(query, s9, s8), tbb::auto_partitioner());
                continue;
            }
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s2, VertexSet &_s4, VertexSet &_s3, VertexSet &_s1) : query{_query}, s2{_s2}, s4{_s4}, s3{_s3}, s1{_s1} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s1.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s1.size(), 1), SetLevel2(query, s2, s4, s3, s1), tbb::auto_partitioner());
                continue;
            }
            ...
            for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) {                  // loop-1 begin
                ...
                for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                    ...
                    if (bitmap_rows) { // full bitmap region
                        ...
                        for (size_t v3_idx = 0; v3_idx < s7.size(); ++v3_idx) {
                            const IdType v3 = s7[v3_idx];
                            VertexSet v3_adj = query.graph->N(v3);
                            VertexSet s8 = s5.remove(v3_adj.vid());
                            VertexSet s9 = s6.intersect(v3_adj);

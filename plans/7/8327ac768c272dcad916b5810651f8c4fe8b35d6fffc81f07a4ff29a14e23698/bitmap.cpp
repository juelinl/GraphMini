...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s12, false, *this, policy, 4, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s10, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_s4, const Bitmap &input_s5, const Bitmap &input_s6, const Bitmap &input_s7)
        : query(query), bitgraph(bitgraph), policy(policy), input_s4(input_s4), input_s5(input_s5), input_s6(input_s6), input_s7(input_s7) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            s9.assign_intersection<bitmap_words, true>(s5, v3_adj);
            s10.assign_removed<bitmap_words, true>(s7, v3_bit_idx);
            counter += BitLevel4<bitmap_words>(query, bitgraph, policy, s8, s9, s10)();
        }
        return counter;
    }
};
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            s5.assign_intersection<bitmap_words, true>(s1, v2_adj);
            s6.assign_removed<bitmap_words, true>(s2, v2_bit_idx);
            s7.assign_intersection<bitmap_words, true>(s2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, s4, s5, s6, s7)();
        }
        return counter;
    }
};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s4, VertexSet &_s5, VertexSet &_s7, VertexSet &_s6) : query{_query}, s4{_s4}, s5{_s5}, s7{_s7}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s9 = s5.intersect(v3_adj);
            VertexSet s10 = s7.remove(v3_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s7 = s2.intersect(v2_adj);
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, s4, s5, s7, s6), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            if (s0.size() == 0)
                continue;
            auto bitmap_rows = BitGraph::build(*query.graph, v0, v0_adj, v0_adj, 13); // bitmap-region build once per anchor

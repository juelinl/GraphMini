  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b7.assign_subtraction<bitmap_words, true>(b6, v4_adj, v4_bit_idx);
            b8.assign_intersection<bitmap_words, true>(b5, v4_adj, v4_bit_idx);
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b3, const Bitmap &input_b4)
        : query(query), bitgraph(bitgraph), policy(policy), input_b3(input_b3), input_b4(input_b4) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b5.assign_subtraction<bitmap_words, true>(b4, v3_adj, v3_bit_idx);
            b6.assign_intersection<bitmap_words, true>(b3, v3_adj, v3_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b3.assign_subtraction<bitmap_words, true>(b2, v2_adj, v2_bit_idx);
            b4.assign_intersection<bitmap_words, true>(b1, v2_adj, v2_bit_idx);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, b3, b4)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s6, VertexSet &_s5) : query{_query}, s6{_s6}, s5{_s5} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s7 = s6.subtract(v4_adj);
            VertexSet s8 = s5.intersect(v4_adj, v4_adj.vid());
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s5 = s4.subtract(v3_adj);
            VertexSet s6 = s3.intersect(v3_adj, v3_adj.vid());
            if (s5.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s5.size(), 1), SetLevel4(query, s6, s5), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s3 = s2.subtract(v2_adj);

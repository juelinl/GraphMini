  ...
  public:
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b10.subtraction_count<bitmap_words>(v5_adj, v5_bit_idx);
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b9, true, *this, policy, 3, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class BitLevel3 {
  ...
  public:
    BitLevel3(const QueryContext &query, const BitGraph &bitgraph, const BitmapTaskPolicy &policy, const Bitmap &input_b3, const Bitmap &input_b4, const Bitmap &input_b5, const Bitmap &input_b6)
        : query(query), bitgraph(bitgraph), policy(policy), input_b3(input_b3), input_b4(input_b4), input_b5(input_b5), input_b6(input_b6) {}
    uint64_t operator()() const {
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            b8.assign_intersection<bitmap_words, true>(b5, v3_adj);
            b9.assign_subtraction<bitmap_words, true>(b6, v3_adj, v3_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc2, bool parallel_task) const {
        ...
        for (; bc2.valid(); bc2.advance()) { // bitmap local-index loop
            ...
            b3.assign_subtraction<bitmap_words, true>(b1, v2_adj, v2_bit_idx, v2_bit_idx);
            b4.assign_intersection<bitmap_words, true>(b1, v2_adj);
            b5.assign_subtraction<bitmap_words, true>(b2, v2_adj, v2_bit_idx);
            b6.assign_intersection<bitmap_words, true>(b2, v2_adj);
            counter += BitLevel3<bitmap_words>(query, bitgraph, policy, b3, b4, b5, b6)();
        }
        return counter;
    }
};
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s7, VertexSet &_s8, VertexSet &_s9) : query{_query}, s7{_s7}, s8{_s8}, s9{_s9} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            for (size_t v5_idx = 0; v5_idx < s11.size(); v5_idx++) { // loop-5 begin
                ...
                counter += s10.subtract_cnt(v5_adj);
            }
        }
    }
};
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s4, VertexSet &_s5, VertexSet &_s6, VertexSet &_s3) : query{_query}, s4{_s4}, s5{_s5}, s6{_s6}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s9 = s6.subtract(v3_adj);
            if (s9.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s9.size(), 1), SetLevel4(query, s7, s8, s9), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s3 = s1.subtract(v2_adj, v2_adj.vid());
            ...
            if (s3.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel3(query, s4, s5, s6, s3), tbb::auto_partitioner());
                continue;
            }

...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s8, false, *this, policy, 1, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s6, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_v2_adj, VertexSet &_s4, VertexSet &_s3) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, v2_adj{_v2_adj}, s4{_s4}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s5 = v3_adj.remove(v0_adj.vid()).remove(v1_adj.vid()).remove(v2_adj.vid());
            if (s5.size() == 0)
                continue;
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s2) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s2{_s2} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s3.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel3(query, v0_adj, v1_adj, v2_adj, s4, s3), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s2.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s2.size(), 1), SetLevel2(query, v0_adj, v1_adj, s2), tbb::auto_partitioner());
                continue;
            }
                ...
                for (size_t v2_idx = 0; v2_idx < s2.size(); v2_idx++) { // loop-2 begin
                    ...
                    for (size_t v3_idx = 0; v3_idx < s3.size(); v3_idx++) { // loop-3 begin
                        ...
                        if (s5.size() == 0)
                            continue;
                        VertexSet s6 = s4.intersect(v3_adj);
                        auto bitmap_rows = BitGraph::build(*query.graph, v3, v3_adj, v3_adj, 4); // bitmap-region build once per anchor
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s5, *bitmap_s6)();
                            };
                            ...
                            for (size_t v4_idx = 0; v4_idx < s6.size(); ++v4_idx) {
                                const IdType v4 = s6[v4_idx];
                                VertexSet v4_adj = query.graph->N(v4);
                                VertexSet s7 = s5.remove(v4_adj.vid());
                                VertexSet s8 = s6.bounded(v4_adj.vid());

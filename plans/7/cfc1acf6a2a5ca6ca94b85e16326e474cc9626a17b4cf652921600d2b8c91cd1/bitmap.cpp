...
class BitLevel5 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s10, false, *this, policy, 1, 0);
    }
...
class BitLevel4 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s8, true, *this, policy, 0, query.nested_thresholds[4]);
    }
    template <class Cursor>
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_s3, VertexSet &_s2, VertexSet &_s1) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, s3{_s3}, s2{_s2}, s1{_s1} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            if (s1.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s1.size(), 1), SetLevel2(query, v0_adj, v1_adj, s3, s2, s1), tbb::auto_partitioner());
                continue;
            }
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) { // loop-1 begin
                ...
                for (size_t v2_idx = 0; v2_idx < s1.size(); v2_idx++) { // loop-2 begin
                    ...
                    VertexSet s5 = s3.remove(v2_adj.vid());
                    VertexSet s6 = s2.intersect(v2_adj);
                    auto bitmap_rows = BitGraph::build(*query.graph, v2, v2_adj, v2_adj, 4); // bitmap-region build once per anchor
                    for (size_t v3_idx = 0; v3_idx < s5.size(); v3_idx++) {                  // loop-3 begin
                        const IdType v3 = s5[v3_idx];
                        VertexSet v3_adj = query.graph->N(v3);
                        VertexSet s7 = s4.remove(v3_adj.vid());
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s7, *bitmap_s8)();
                            };
                            ...
                            for (size_t v4_idx = 0; v4_idx < s8.size(); ++v4_idx) {
                                const IdType v4 = s8[v4_idx];
                                VertexSet v4_adj = query.graph->N(v4);
                                VertexSet s9 = s7.remove(v4_adj.vid());
                                VertexSet s10 = s8.bounded(v4_adj.vid());

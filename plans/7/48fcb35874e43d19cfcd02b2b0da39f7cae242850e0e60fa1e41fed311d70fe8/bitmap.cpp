...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s7, VertexSet &_s6, VertexSet &_s5) : query{_query}, s7{_s7}, s6{_s6}, s5{_s5} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s5 = s3.bounded(v2_adj.vid());
            VertexSet s6 = s2.intersect(v2_adj);
            VertexSet s7 = s4.remove(v2_adj.vid());
            if (s5.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s5.size(), 1), SetLevel3(query, s7, s6, s5), tbb::auto_partitioner());
                continue;
            }

...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_v0_adj, VertexSet &_v1_adj, VertexSet &_v2_adj, VertexSet &_s5, VertexSet &_s7, VertexSet &_s6) : query{_query}, v0_adj{_v0_adj}, v1_adj{_v1_adj}, v2_adj{_v2_adj}, s5{_s5}, s7{_s7}, s6{_s6} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s8 = v3_adj.remove(v0_adj.vid()).remove(v1_adj.vid()).remove(v2_adj.vid());
            if (s8.size() == 0)
                continue;
            ...
            for (size_t v4_idx = 0; v4_idx < s10.size(); v4_idx++) { // loop-4 begin
                ...
                counter += 1ll * s12.size() * s11.size();
                /* Val: 1 | Group: (0),(1) | Comp: |VSet(12)|*|VSet(11)| */
                counter += -1ll * s12.intersect_cnt(s11);
                /* Val: -1 | Group: (0 1) | Comp: |VSet(12) & VSet(11)| */
            }
        }
    }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s6.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s6.size(), 1), SetLevel3(query, v0_adj, v1_adj, v2_adj, s5, s7, s6), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s4 = s1.intersect(v1_adj);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            if (s0.size() > query.nested_thresholds[1]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s0.size(), 1), SetLevel1(query, v0_adj, s1, s0), tbb::auto_partitioner());
                continue;
            }

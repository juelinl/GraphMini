...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s8, VertexSet &_s9, VertexSet &_s7) : query{_query}, s8{_s8}, s9{_s9}, s7{_s7} {};
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            if (s7.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel4(query, s8, s9, s7), tbb::auto_partitioner());
                continue;
            }

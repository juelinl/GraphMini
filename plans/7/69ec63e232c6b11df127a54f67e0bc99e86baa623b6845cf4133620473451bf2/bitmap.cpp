  ...
  public:
    ...
    uint64_t operator()(Cursor bc5, bool parallel_task) const {
        ...
        for (; bc5.valid(); bc5.advance()) { // bitmap local-index loop
            ...
            counter += b13.removed_count<bitmap_words>(v5_bit_idx);
  ...
  public:
    ...
    uint64_t operator()(Cursor bc4, bool parallel_task) const {
        ...
        for (; bc4.valid(); bc4.advance()) { // bitmap local-index loop
            ...
            b12.assign_removed<bitmap_words, true>(b11, v4_bit_idx);
            const auto v4_adj = bitgraph.local_row(v4_bit_idx);
            b13.assign_intersection<bitmap_words, true>(b9, v4_adj);
...
class SetLevel4 {
  ...
  public:
    SetLevel4(const QueryContext &_query, VertexSet &_s11, VertexSet &_s9, VertexSet &_s10) : query{_query}, s11{_s11}, s9{_s9}, s10{_s10} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v4_idx = r.begin(); v4_idx < r.end(); v4_idx++) { // loop-4begin
            ...
            VertexSet s12 = s11.remove(v4_adj.vid());
            VertexSet s13 = s9.intersect(v4_adj);
...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s5, VertexSet &_s6, VertexSet &_s8, VertexSet &_s7) : query{_query}, s5{_s5}, s6{_s6}, s8{_s8}, s7{_s7} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v3_idx = r.begin(); v3_idx < r.end(); v3_idx++) { // loop-3begin
            ...
            VertexSet s11 = s8.remove(v3_adj.vid());
            if (s10.size() > query.nested_thresholds[4]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s10.size(), 1), SetLevel4(query, s11, s9, s10), tbb::auto_partitioner());
                continue;
            }
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            if (s7.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel3(query, s5, s6, s8, s7), tbb::auto_partitioner());
                continue;
            }

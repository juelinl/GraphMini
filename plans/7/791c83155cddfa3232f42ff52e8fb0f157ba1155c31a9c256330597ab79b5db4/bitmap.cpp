...
class SetLevel3 {
  ...
  public:
    SetLevel3(const QueryContext &_query, VertexSet &_s6, VertexSet &_s5, VertexSet &_s7) : query{_query}, s6{_s6}, s5{_s5}, s7{_s7} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
...
class SetLevel2 {
  ...
  public:
    SetLevel2(const QueryContext &_query, VertexSet &_s1, VertexSet &_s2, VertexSet &_s4, VertexSet &_s3) : query{_query}, s1{_s1}, s2{_s2}, s4{_s4}, s3{_s3} {};
    void operator()(const tbb::blocked_range<size_t> &r) const {
        auto &ctx = query.ctx;
        const int worker_id = tbb::this_task_arena::current_thread_index();
        cc &counter = ctx.per_thread_result.at(worker_id);
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            VertexSet s7 = s4.remove(v2_adj.vid());
            if (s7.size() > query.nested_thresholds[3]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s7.size(), 1), SetLevel3(query, s6, s5, s7), tbb::auto_partitioner());
                continue;
            }
  ...
  public:
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v1_idx = r.begin(); v1_idx < r.end(); v1_idx++) { // loop-1begin
            ...
            VertexSet s3 = v1_adj.bounded(v0_adj.vid());
            if (s3.size() == 0)
                continue;
            VertexSet s4 = s0.intersect(v1_adj);
            if (s3.size() > query.nested_thresholds[2]) {
                tbb::parallel_for(tbb::blocked_range<size_t>(0, s3.size(), 1), SetLevel2(query, s1, s2, s4, s3), tbb::auto_partitioner());
                continue;
            }
                    ...
                    for (size_t v3_idx = 0; v3_idx < s7.size(); v3_idx++) {                  // loop-3 begin
                        ...
                        if (bitmap_rows) { // full bitmap region
                            ...
                            auto bitmap_execute = [&](auto bitmap_tag) {
                                ...
                                return BitLevel4<bitmap_words>(query, bitgraph, bitmap_task_policy, *bitmap_s8, *bitmap_s9)();
                            };

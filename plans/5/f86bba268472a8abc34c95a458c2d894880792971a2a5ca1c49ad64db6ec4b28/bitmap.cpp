...
struct QueryContext {
    ...
    const std::array<size_t, 4> nested_thresholds;
};
...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_s3, false, *this, policy, 2, 0);
    }
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            counter += s3.bounded_count<bitmap_words>(v3_bit_idx);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            for (size_t v3_idx = 0; v3_idx < s3.size(); v3_idx++) { // loop-3 begin
                ...
                counter += s3.bounded_cnt(v3_adj.vid());
            }
        }
    }
};
...
void plan(const GraphType *graph, Context &ctx) {
    ...
    const QueryContext query{graph, ctx, progress, {0, nested_threshold(graph->num_vertex, graph->num_edge, graph->max_degree, 4), nested_threshold(graph->num_vertex, graph->num_edge, graph->max_degree, 4), 0}};

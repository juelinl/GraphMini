    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v0 = r.begin(); v0 < r.end(); v0++) { // loop-0begin
            ...
            for (size_t v1_idx = 0; v1_idx < s0.size(); v1_idx++) { // loop-1 begin
                ...
                /* Val: 1 | Group: (0),(1),(2) | Comp: |VSet(1)|*|VSet(1)|*|VSet(1)| */
                counter += -1ll * s1.size() * s1.size();
                /* Val: -1 | Group: (0 2),(1) | Comp: |VSet(1) & VSet(1)|*|VSet(1)| */
                counter += -1ll * s1.size() * s1.size();
                /* Val: -1 | Group: (0),(1 2) | Comp: |VSet(1)|*|VSet(1) & VSet(1)| */
                counter += -1ll * s1.size() * s1.size();
                /* Val: -1 | Group: (0 1),(2) | Comp: |VSet(1) & VSet(1)|*|VSet(1)| */
                counter += 2ll * s1.intersect(s1).intersect_cnt(s1);
                /* Val: 2 | Group: (0 1 2) | Comp: |VSet(1) & VSet(1) & VSet(1)| */
            }
        }
    }
};
...
void plan(const GraphType *graph, Context &ctx) {
    ...
    const QueryContext query{graph, ctx, progress, {0, 0, 0, 0}};

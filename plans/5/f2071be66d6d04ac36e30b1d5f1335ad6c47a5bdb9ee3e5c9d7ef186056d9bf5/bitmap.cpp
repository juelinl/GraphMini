...
class BitLevel3 {
  ...
  public:
    ...
    uint64_t operator()() const {
        return bitmap_for_each(input_b2, false, *this, policy, 2, 0);
    }
    ...
    uint64_t operator()(Cursor bc3, bool parallel_task) const {
        ...
        for (; bc3.valid(); bc3.advance()) { // bitmap local-index loop
            ...
            counter += b2.subtraction_count<bitmap_words>(v3_adj, v3_bit_idx, v3_bit_idx);
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const {
        ...
        for (size_t v2_idx = r.begin(); v2_idx < r.end(); v2_idx++) { // loop-2begin
            ...
            for (size_t v3_idx = 0; v3_idx < s2.size(); v3_idx++) { // loop-3 begin
                ...
                counter += s2.subtract_cnt(v3_adj, v3_adj.vid());
            }
        }
    }
};

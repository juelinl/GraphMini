...
bool try_parse_scheduler_type(const std::string &value, SchedulerType &scheduler_type) {
    ...
    if (lowered == "iep_first") {
        scheduler_type = SchedulerType::IepFirst;
        return true;
    }
...
std::string build_run_help(const cxxopts::Options &) {
    ...
    out << "  ./build/bin/run --graph_name=<name> --path_to_graph=<dir> --query_name=<name> "
           ...
           "[--scheduler=<graphpi|graphmini|graphzero|outgoing|bitmap_balanced|iep_first>] [--num_threads=<count>] "
    ...
    out << "  --scheduler       graphpi, graphmini, graphzero; experimental: outgoing, bitmap_balanced, iep_first. Default: graphmini.\n";
...
int main(int argc, char *argv[]) {
    ...
    options.add_options()
            ...
            ("scheduler", "Scheduler: graphpi|graphmini|graphzero|outgoing|bitmap_balanced|iep_first",

...
bool eager_next_use(const PlanIR &plan, const MiniGraphIR &mg) {
    const auto &next = plan.iter_set.at(mg.loop_depth());
    if (!(next == mg.m_vertices))
        return false;
    for (const auto &op : plan.set_ops.at(mg.loop_depth() + 1))
        if (mg.computed(next, op))
            return true;
    return false;
}
ReuseEstimate estimate_visits(const PlanIR &plan, const MiniGraphIR &mg, int depth, int uses) {
    ReuseEstimate out{plan.iter_set.at(mg.loop_depth()).id, {}, uses};
    for (int dep = mg.loop_depth() + 1; dep < depth; ++dep) {
        const auto &iter = plan.iter_set.at(dep);
        const auto parent = plan.get_parent_vset(iter, mg.loop_depth());
        if (parent) {
            const double p1 = 1.0 * plan.meta.num_edge / plan.meta.num_vertex / plan.meta.num_vertex;
            const double p2 = 1.0 * plan.meta.num_triangle * 6 * plan.meta.num_vertex /
                              plan.meta.num_edge / plan.meta.num_edge;
            double rate = 1.0;
            for (int adj = mg.loop_depth(); adj < iter.loop_depth(); ++adj)
                rate *= plan.iter_set.at(adj).share_at_least_one_parent_node(*parent) ? p2 : p1;
            out.factors.push_back({parent->id, rate});
        } else
            out.factors.push_back({{}, 1.0 * plan.meta.num_edge / plan.meta.num_vertex});
    }
    return out;
}
} // namespace
    ...
    for (const auto &level : plan.mg_ops) {
        for (const auto &mg : level) {
            ...
            MiniGraphExecution out{mg.id,
                                   ...
                                   mg.vset_id(),
                                   mg.vint_id(),
                                   plan.iter_set.at(mg.loop_depth()).id,
                                   parent ? std::optional<int>(parent->id) : std::nullopt,
                                   eager,
                                   plan.is_bounded(mg),
                                   plan.is_par(mg),
                                   plan.config.pruningType == PruningType::CostModel && !eager,
                                   {},
                                   {}};
            for (const auto &iter : plan.iter_set)
                out.direct_indices.emplace(iter.id, shares_indices(mg, iter));
            if (out.estimate_reuse) {
                const int last = std::min(plan.p_size - 2, plan.p_size - plan.iep_num - 1);

...
VertexSetIR::VertexSetIR(const EdgeIR &edges, const EdgeRestrictIR restricts, int loop_depth,
                         AdjMatType query_type)
    : m_loop_depth{loop_depth}, adjMatType{query_type} {
...
bool VertexSetIR::share_at_least_one_parent_node(const VertexSetIR &other) const {
    ...
    for (int dep = 0; dep <= max_dep; dep++) {
        if (is_edge(dep) && other.is_edge(dep))
            return true;
    }
    return false;
}
...
bool VertexSetIR::operator==(const VertexSetIR &rhs) const {
    return m_edges == rhs.m_edges && m_restricts == rhs.m_restricts && m_loop_depth == rhs.m_loop_depth;
}
...
bool VertexSetIR::same_iep_computation(const VertexSetIR &rhs) const {
    ...
    default:
        ...
        for (int i = 0; i <= min_dep; ++i) {
            if (is_edge(i) != rhs.is_edge(i) || is_restricted(i) != rhs.is_restricted(i))
                return false;
        }
        return true;
    }
...
bool VertexSetIR::is_superset_of(const VertexSetIR &rhs) const {
    if (m_loop_depth > rhs.m_loop_depth)
        return false;
    for (int i = 0; i <= m_loop_depth; i++) {
        if (adjMatType == AdjMatType::VertexInduced) {
            if ((rhs.is_edge(i) != is_edge(i)) || (!rhs.is_restricted(i) && is_restricted(i))) {
...
bool MiniGraphIR::operator==(const MiniGraphIR &rhs) const {
    return m_intersect == rhs.m_intersect && m_vertices == rhs.m_vertices;
}

bool MiniGraphIR::operator<(const MiniGraphIR &rhs) const {
    if (rhs == *this)
        return false;
    else if (m_vertices < rhs.m_vertices)
        return true;
    else if (rhs.m_vertices < m_vertices)
        return false;
    else if (m_intersect < rhs.m_intersect)
        return true;
    else
        return false;
}
...
bool MiniGraphIR::is_superset_of(const MiniGraphIR &rhs) const {
    return m_vertices.is_superset_of(rhs.m_vertices) && m_intersect.is_superset_of(rhs.m_intersect);
}

bool MiniGraphIR::is_superset_of(const VertexSetIR &vertices, const VertexSetIR &intersect) const {
    return m_vertices.is_superset_of(vertices) && m_intersect.is_superset_of(intersect);
}
...
bool MiniGraphIR::computed(const VertexSetIR &vertices, const VertexSetIR &intersect) const {
    ...
    for (int i = 0; i <= m_intersect.loop_depth(); ++i) {
        if (m_intersect.is_edge(i) != intersect.is_edge(i))
            return false;
        if (m_intersect.is_restricted(i) != intersect.is_restricted(i))
            return false;
    }
    return true;
}

...
namespace minigraph {
    struct Graph {
        ...
        uint64_t compute_degstd() {
            deg_std = 0;
            for (size_t v_id = 0; v_id < num_vertex; v_id++) {
                auto deg = Degree(v_id);
                deg_std += deg * deg;
            }

            deg_std = sqrt(deg_std) / num_vertex;
        }
        uint64_t get_degstd() const { return deg_std;};
